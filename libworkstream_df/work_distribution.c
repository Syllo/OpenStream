...
void reorder_pushes(wstream_df_thread_p cthread) {
  ...
  for (i = 0; i < NUM_PUSH_SLOTS; i++) {
    ...
    cthread->push_reorder_slots[insert_pos].cost =
        mem_cache_misses(cthread) - import->cache_misses[cthread->worker_id];
    insert_pos++;
  ...
  if (insert_pos > 0) {
    ...
    qsort(cthread->push_reorder_slots, insert_pos, sizeof(wstream_df_frame_cost_t),
          compare_frame_costs);
...
int work_push_beneficial_max_writer(wstream_df_frame_p fp, wstream_df_thread_p cthread,
                                    int num_workers, unsigned *target_worker) {
...
int work_push_beneficial_owner(wstream_df_frame_p fp, wstream_df_thread_p cthread, int num_workers,
                               int *target_worker) {
  unsigned int max_worker;
  int numa_node_id;
  int max_data;
...
int work_push_beneficial_split_owner(wstream_df_frame_p fp, wstream_df_thread_p cthread,
                                     int num_workers, int *target_worker) {
  unsigned int max_worker;
  int numa_node_id;
  int max_data;
  wstream_df_numa_node_p numa_node;
  ...
  if (max_data > PUSH_MIN_REL_FRAME_SIZE && numa_node_id != -1 &&
      cthread->numa_node->id != numa_node_id) {
    /* Choose random worker sharing the target node */
    numa_node = numa_node_by_id(numa_node_id);
    rand_idx = prng_nextn(&cthread->rands, numa_node->num_workers);
    max_worker = numa_node->workers[rand_idx]->worker_id;
...
int work_push_beneficial_split_owner_chain(wstream_df_frame_p fp, wstream_df_thread_p cthread,
                                           int num_workers, int *target_worker) {
  unsigned int max_worker;
  int numa_node_id;
  int max_data;
...
int work_push_beneficial_split_owner_chain_inner_mw(wstream_df_frame_p fp,
                                                    wstream_df_thread_p cthread, int num_workers,
                                                    unsigned int *target_worker) {
  unsigned int max_worker;
  int numa_node_id;
  int max_data;
  size_t data[num_numa_nodes];
...
int work_push_beneficial_split_score_nodes(wstream_df_frame_p fp, wstream_df_thread_p cthread,
                                           int num_workers, unsigned int *target_worker) {
  unsigned int max_worker;
  int numa_node_id;
  size_t data[num_numa_nodes];
 ...
 * returned.
 */
int work_push_beneficial(wstream_df_frame_p fp, wstream_df_thread_p cthread, int num_workers,
                         wstream_df_thread_p *wstream_df_worker_threads, int *target_worker) {
  int res;
  ...
  res =
      work_push_beneficial_split_owner_chain_inner_mw(fp, cthread, num_workers, &lcl_target_worker);
...
#error "No push strategy defined" */
  ...
  if (/* Only migrate to a different worker */
      ...
      level_of_common_ancestor(cthread->cpu, wstream_df_worker_threads[lcl_target_worker]->cpu) >=
          PUSH_MIN_MEM_LEVEL) {
 ...
 */
int work_try_push(wstream_df_frame_p fp, int target_worker, wstream_df_thread_p cthread,
...
static wstream_df_frame_p steal_hwloc_pu(wstream_df_thread_p thief, hwloc_obj_t current_obj,
                                         hwloc_obj_t *stolen_from) {
  if (current_obj->type != HWLOC_OBJ_PU)
    return NULL;
  wstream_df_thread_p pu_worker_thread = (wstream_df_thread_p)current_obj->userdata;
...
static hwloc_obj_t random_hwloc_pu_in_subtree(wstream_df_thread_p thief, hwloc_obj_t root) {
 ...
 */
static wstream_df_frame_p steal_hwloc_bottom_up(wstream_df_thread_p thief, hwloc_obj_t current_obj,
                                                hwloc_obj_t stop_at, hwloc_obj_t *stolen_from) {
  ...
  while (current_obj != stop_at && !frame) {
    ...
    while (current_obj != stop_at && !current_obj->next_sibling && !current_obj->prev_sibling) {
...
static wstream_df_frame_p work_steal(wstream_df_thread_p cthread,
                                     wstream_df_thread_p *wstream_df_worker_threads) {
  ...
  if (cthread->last_steal_from != NULL) {
    wstream_df_thread_p stolen_from = (wstream_df_thread_p)cthread->last_steal_from->userdata;
...
#if WQUEUE_PROFILE
      unsigned common_level = level_of_common_ancestor(cthread->cpu, stolen_from);
      inc_wqueue_counter(&cthread->steals_mem[common_level], 1);
#if ALLOW_WQEVENT_SAMPLING
      wstream_df_thread_p stolen_worker_thread = (wstream_df_thread_p)stolen_from->userdata;
      trace_steal(cthread, stolen_worker_thread->worker_id, stolen_from->logical_index, fp->size,
                  fp);
#endif
  ...
  if (fp != NULL) {
    ...
    for (unsigned worker_id = 0; worker_id < wstream_num_workers; worker_id++) {
      if (fp->bytes_cpu_in[worker_id]) {
        unsigned level =
            level_of_common_ancestor(cthread->cpu, wstream_df_worker_threads[worker_id]->cpu);
        inc_wqueue_counter(&cthread->bytes_mem[level], fp->bytes_cpu_in[worker_id]);
        inc_transfer_matrix_entry(cthread->worker_id, worker_id, fp->bytes_cpu_in[worker_id]);
      }
    }
  }
