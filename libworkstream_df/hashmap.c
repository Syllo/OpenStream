...
static int hashmap_rehash(struct hashmap *map, size_t new_size)
{
    ...
    new_table = (struct hashmap_entry *)slab_alloc(
        current_thread, current_thread->slab_cache,
        new_size * sizeof(struct hashmap_entry));
    if (!new_table) {
        return -ENOMEM;
    }
    ...
    slab_free(current_thread->slab_cache, old_table);
    return 0;
revert:
    map->table_size = old_size;
    map->table = old_table;
    slab_free(current_thread->slab_cache, new_table);
    return -EINVAL;
}
    ...
    size_t initial_size)
{
    ...
    map->num_entries = 0;
    map->table = (struct hashmap_entry *)slab_alloc(
        current_thread, current_thread->slab_cache,
        initial_size * sizeof(struct hashmap_entry));
    if (!map->table) {
        return -ENOMEM;
    }
...
void hashmap_destroy(struct hashmap *map)
{
    ...
    slab_free(current_thread->slab_cache, map->table);
